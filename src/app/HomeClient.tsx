...
import {
  ...
  ArrowRight, CheckCircle, Star, Users, Newspaper, Calendar, User,
} from "lucide-react";
...
function formatDateSimple(iso: string) {
  return new Date(iso).toLocaleDateString("kk-KZ", { year: "numeric", month: "long", day: "numeric" });
}

export default function HomeClient({ latestArticles }: { latestArticles: LatestArticle[] }) {
  ...
  const latestTitle = lang === "ru" ? "Последние публикации" : "Соңғы жарияланымдар";
  const latestSubtitle = lang === "ru"
    ? "Недавно опубликованные статьи педагогов"
    : "Жақында жарияланған педагогтар мақалалары";
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="max-w-3xl">
            ...
            <div className="flex flex-wrap gap-3">
              <Link href="/services" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white text-blue-800 font-semibold hover:bg-blue-50 transition-colors shadow-lg">
                <FileText className="w-5 h-5" />
                {t("home.hero.btn.publish")}
              </Link>
              <Link href="/editor-service" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-amber-400 text-amber-900 font-semibold hover:bg-amber-300 transition-colors shadow-lg">
                <PenTool className="w-5 h-5" />
                {t("home.hero.btn.editor")}
              </Link>
              <Link href="/journals" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl border-2 border-white/40 text-white font-semibold hover:bg-white/10 transition-colors">
      ...
      <section className="py-16 lg:py-20 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {features.map((f) => (
              <Link key={f.href} href={f.href} className="group relative bg-white border border-gray-100 rounded-2xl p-6 card-shadow hover:card-shadow-hover transition-all duration-300 hover:-translate-y-1">
        ...
        <section className="py-16 lg:py-20 bg-white">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex items-center justify-between mb-10">
              <div>
                <h2 className="text-3xl font-bold text-gray-900 mb-2">{latestTitle}</h2>
                <p className="text-gray-500">{latestSubtitle}</p>
              </div>
              <Link href="/archive" className="hidden sm:flex items-center gap-1 text-blue-600 font-medium hover:text-blue-800 transition-colors">
                {lang === "ru" ? "Все публикации" : "Барлық мақалалар"}
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
              {latestArticles.map((a) => (
                <Link
                  ...
                  className="group bg-white border border-gray-100 rounded-2xl p-5 card-shadow hover:card-shadow-hover transition-all duration-200 hover:-translate-y-0.5 flex flex-col"
                >
                  ...
                  <div className="space-y-1 text-xs text-gray-500 flex-1">
                    ...
                    <p className="line-clamp-1 text-gray-400">{a.journalName}</p>
                    <p className="flex items-center gap-1.5 text-gray-400">
                      <Calendar className="w-3 h-3 shrink-0" />
                      {formatDateSimple(a.publishedAt)}
                    </p>
                  </div>
                  <div className="mt-3 pt-3 border-t border-gray-100 flex items-center text-blue-600 text-xs font-medium">
                    {lang === "ru" ? "Читать статью" : "Мақаланы оқу"}
      ...
      <section className="py-16 lg:py-20 bg-slate-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between mb-10">
            ...
            <Link href="/journals" className="hidden sm:flex items-center gap-1 text-blue-600 font-medium hover:text-blue-800 transition-colors">
              {t("home.journals.all")}
              <ArrowRight className="w-4 h-4" />
            </Link>
          </div>
      ...
      <section className="py-16 bg-white">
        <div className="max-w-2xl mx-auto px-4 text-center">
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            <Link href="/publish" className="inline-flex items-center justify-center gap-2 px-8 py-3.5 rounded-xl bg-blue-700 text-white font-semibold hover:bg-blue-800 transition-colors shadow-lg">
              <FileText className="w-5 h-5" />
              {t("home.cta.btn.publish")}
            </Link>
            <Link href="/editor-service" className="inline-flex items-center justify-center gap-2 px-8 py-3.5 rounded-xl border-2 border-blue-700 text-blue-700 font-semibold hover:bg-blue-50 transition-colors">
              <PenTool className="w-5 h-5" />
