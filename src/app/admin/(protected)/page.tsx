...
export default async function AdminDashboard() {
  ...
  return (
    <div className="p-6 lg:p-8">
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-gray-900">Ð‘Ð°ÑÑ‚Ñ‹ Ð±ÐµÑ‚</h1>
        <p className="text-gray-500 text-sm mt-1">USTAZALEMI ÐÐ´Ð¼Ð¸Ð½ ÐŸÐ°Ð½ÐµÐ»ÑŒ</p>
      </div>

      {/* Stats */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        {[
          { label: "Ð‘Ð°Ñ€Ð»Ñ‹Ò› Ó©Ñ‚Ñ–Ð½Ñ–Ð¼Ð´ÐµÑ€", value: totalSubmissions, icon: ClipboardList, color: "blue", href: "/admin/submissions" },
          { label: "Ð–Ð°Ñ€Ð¸ÑÐ»Ð°Ð½Ð´Ñ‹", value: totalArticles, icon: FileText, color: "green", href: "/admin/articles" },
          { label: "Ð¡ÐµÑ€Ñ‚Ð¸Ñ„Ð¸ÐºÐ°Ñ‚Ñ‚Ð°Ñ€", value: totalCerts, icon: Award, color: "amber", href: "/admin/certificates" },
          { label: "Ð¢Ó©Ð»ÐµÐ¼ ÐºÒ¯Ñ‚Ñ–Ð»ÑƒÐ´Ðµ", value: pendingPayment, icon: Clock, color: "red", href: "/admin/submissions?status=waiting_payment" },
        ].map((stat) => (
      ...
      <div className="bg-white rounded-2xl card-shadow">
        <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
          <h2 className="font-bold text-gray-900">Ð¡Ð¾Ò£Ò“Ñ‹ Ó©Ñ‚Ñ–Ð½Ñ–Ð¼Ð´ÐµÑ€</h2>
        ...
        <div className="divide-y divide-gray-50">
          {recentSubmissions.length === 0 ? (
            <div className="px-6 py-10 text-center text-gray-400">Ó¨Ñ‚Ñ–Ð½Ñ–Ð¼ Ð¶Ð¾Ò›</div>
          ) : (
            recentSubmissions.map((sub) => (
              <div key={sub.id} className="px-6 py-4 flex items-center gap-4">
                ...
                <div className="flex items-center gap-2 shrink-0">
                  ...
                  <span className="font-bold text-blue-700 text-sm">{sub.price.toLocaleString()} Ñ‚Ð³</span>
                </div>
                <Link
